...
url = "https://setwithfriends.com/game/skillful-slippery-chalk-12"
...
while running:
    if keyboard.is_pressed('ctrl'):
        if first_run:
            # the jss number to draw the cards changes for some reason sometimes and here i am clumsily deriving it
            # (i can think of a lot of scenarios where i can totally break)
            page_source = driver.page_source
            html = BeautifulSoup(page_source, "html.parser")
            find_arr = []
            for i in range(100):
                if str(html).find('<svg class="jss'+str(i+1)) != -1:
                    find_arr.append(i + 1)
            jss_num = max(find_arr)
            first_run = False
        update_info()
    ...
    if keyboard.is_pressed('alt'):
        update_info()
        print(html.prettify())
        # jss_counter = html.find_all("jss")
        # for jss in jss_counter:
        #    print(jss)
        # print("jss ran!!!")
